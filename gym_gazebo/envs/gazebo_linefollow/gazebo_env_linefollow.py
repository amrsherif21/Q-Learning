...
class Gazebo_Linefollow_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
        ...
        bin_width = gray_image.shape[1] // NUM_BINS

        for i in range(NUM_BINS):
            section = gray_image[:, i*bin_width:(i+1)*bin_width]

            if np.mean(section) < 200:  # Assuming line is darker than the road
                state[i] = 1 

        for i in range(1, NUM_BINS):
            if state[i]:
                cv2.line(cv_image, (i*bin_width, 0), (i*bin_width, gray_image.shape[0]), (0, 0, 255), 1)
            else:
                cv2.line(cv_image, (i*bin_width, 0), (i*bin_width, gray_image.shape[0]), (255, 255, 255), 1)

        font = cv2.FONT_HERSHEY_SIMPLEX
        state_text = str(state)
        cv2.putText(cv_image, state_text, (10, 30), font, 0.5, (255, 255, 255), 1, cv2.LINE_AA)

        cv2.imshow('window', cv_image)
        cv2.waitKey(1)
        if sum(state) == len(state): 
             print("All state lines active. Ending episode.")
             return state, True 
        # Check for episode termination
        if sum(state) == 0:  # No line detected in the image
            self.timeout += 1
            if self.timeout > 15:
                done = True
            else:
